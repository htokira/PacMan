...
class Ghost:
    ...
    def handle_player_collision(self):
        ...
        Returns:
            tuple: (ghost_eaten, player_killed) —  логічні значення,
            що вказують чи був з'їдений привид та чи загинув гравець.
        """
        if self.mode == "RETURNING":
            return False, False
    ...
    def find_closest_direction(self, valid_dirs, player):
        ...
        for d in valid_dirs:
            dist = self.calculate_distance(self.rect.x + d[0],
                                           self.rect.y + d[1], player.rect.centerx, player.rect.centery)
